...
public struct SubtitleView: View {
    ...
    public var body: some View {
        VStack(spacing: 0) {
            ...
            .padding(.top, 16)
            .padding(.bottom, 8)
            .background(Color.clear) // 纯透明

            Divider().background(Color.white.opacity(0.15))

            // ================= 2. 50/50 均分区块 (左上角完全对齐) =================
            GeometryReader { geometry in
                VStack(spacing: 0) {
                    
                    // -------- 上半部分：对方说 (50% 高度, 纯正左上角对齐, 透明暗青) --------
        ...
        .background(Color.clear) // 彻底移除底部黑框遮罩！
    }
}
'''
...
print("✅ SubtitleView.swift 已更新！纯透明玻璃顶栏、左上角顶格对齐、双透明块无缝拼接已生效。")
